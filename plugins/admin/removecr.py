...
def panel(_, message):    
    try:
        ...
        if len(data) < 3: return message.reply('usalo /removecr <id> <cantidad>')

        MongoClient().removecr(int(data[1]), int(data[2]))

        message.reply(f'Se han quitado {data[2]} CR a {data[1]}')
