...
from pages.braintree_avs import BraintreeAuthWoo
...
def panel(_, message):
    ...
    comando_info = MongoClient().collection_cuatro.find_one({"comando": "/bvs"})
    print("Estado del gate 'on' en la base de datos:", comando_info)  
    
    if comando_info is None or comando_info.get('estado') != '✅':
        return message.reply(f"""<b>Command: <code>$bsv</code>\nGateway: <code>Braintree AVS</code>\nEstado: <code>OFF❌</code>\nFormat: <code>/bsv cc|month|year|cvv.</code></b>
""")
    ...
    if '<b>ingrese la ccs.</b>' in cc:
        return message.reply(f"""<b>Command: <code>$bsv</code>\nGateway: <code>Braintree AVS</code>\nEstado: <code>✅</code>\nFormat: <code>/bvs cc|month|year|cvv.</code></b>
""")
