...
from pages.braintree_ccn import BraintreeAuthWoo
...
def panel(_, message):
    ...
    comando_info = MongoClient().collection_cuatro.find_one({"comando": "/ccn"})
    print("Estado del gate 'on' en la base de datos:", comando_info)  
    
    if comando_info is None or comando_info.get('estado') != '✅':
        return message.reply(f"""<b>Command: <code>$ccn</code>\nGateway: <code>Braintree CCN</code>\nEstado: <code>OFF❌</code>\nFormat: <code>/ccn cc|month|year|cvv.</code></b>
""")
