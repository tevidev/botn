...
def panel(_, message):
    ...
    comando_info = MongoClient().collection_cuatro.find_one({"comando": "/po"})
    print("Estado del gate 'on' en la base de datos:", comando_info)  
    
    if comando_info is None or comando_info.get('estado') != '✅':
        return message.reply('<b>Command: <code>$po</code>\nGateway: <code>Shopify+Braintree</code>\nEstado: <code>OFF❌</code>\nFormat: <code>/po cc|month|year|cvv.</code></b>')
